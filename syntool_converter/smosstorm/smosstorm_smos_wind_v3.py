...
#     """
#     import glob
#     patt = '/home/cercache/project/catds/cecos/smos_storm/outputV3/NETCDF/2014/07/*.nc'
#     for path in glob.glob(patt):
#         # print os.path.basename(path)
#         smosstorm_smos_wind_v3(path, '/tmp')
#         # dataset = Dataset(path)
#         # print dataset.variables['longitude'].shape


def smosstorm_smos_wind_v3(infile, outdir,
                           vmin=0., vmax=50.8, vmin_pal=0., vmax_pal=50.):
    """
    """
    # Read/Process data
    dataset = Dataset(infile)
    wind = dataset.variables['wind_speed'][:, :, 0].transpose()
    ...
    lat = dataset.variables['latitude'][lat_slice]
    lon = dataset.variables['longitude'][lon_slice]
    if lon.max() - lon.min() < 180:
        wind = wind[lat_slice, lon_slice]
    else:
        nlon = dataset.variables['longitude'].shape[0]
        lon_first = dataset.variables['longitude'][0]
        lon_last = dataset.variables['longitude'][-1]
        ...
        lon = np.hstack((dataset.variables['longitude'][lon_slicer],
                         dataset.variables['longitude'][lon_slicel] + 360))
    if lat[-1] > lat[0]:
        wind = wind[::-1, :]
        lat = lat[::-1]
    start_time = datetime.strptime(dataset.time_coverage_start, '%Y%m%dT%H%M%SZ')
    stop_time = datetime.strptime(dataset.time_coverage_end, '%Y%m%dT%H%M%SZ')
    ...
    (dtime, time_range) = stfmt.format_time_and_range(start_time, stop_time,
                                                      units='ms')
    metadata['product_name'] = 'SMOSSTORM_SMOS_wind_V3'
